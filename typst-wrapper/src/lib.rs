...
static PACKAGE_TYPST_TOML: &str = include_str!("../tonguetoquill-usaf-memo-package/typst.toml");
static PACKAGE_LIB_TYP: &str = include_str!("../tonguetoquill-usaf-memo-package/lib.typ");
static PACKAGE_UTILS_TYP: &str = include_str!("../tonguetoquill-usaf-memo-package/utils.typ");
    ...
    fn resolve_package_file(&self, spec: &PackageSpec, path: &str) -> Option<&'static str> {
        ...
        if spec.namespace == "preview" && spec.name == "tonguetoquill-usaf-memo" && spec.version.to_string() == "0.0.3" {
            match path {
                ...
                "lib.typ" => Some(PACKAGE_LIB_TYP),
                "utils.typ" => Some(PACKAGE_UTILS_TYP),
                _ => None,
            }
        } else {
            None
